            ...
            deswebs.forEach(desweb => {
                if(desweb.titlecss == undefined){
                    ...
                    deswebElement.innerHTML = `
                        <h4>${desweb.titleoffice}</h4>
                        <img src="${desweb.coveroffice}" alt="${desweb.titleoffice}" class="deswebImg" style="cursor:pointer;">
                        <p><strong>Nivel:</strong> ${desweb.niveloffice}</p>
                    `;
    
                    deswebElement.addEventListener('click', () => {
                        localStorage.setItem('videoDetails', JSON.stringify(desweb));
                        window.location.href = `./itemDetailCss.html?id=${desweb.titleoffice}`;
                    });
        ...
        function displayDeswebs(deswebs) {
            ...
            deswebs.forEach(desweb => {
                ...
                deswebElement.innerHTML = `
                    <h4>${desweb.titleoffice}</h4>
                    <p>Dificultad: ${desweb.niveloffice}</p>
                    <img src="${desweb.coveroffice}" alt="${desweb.titleoffice}" class="deswebImg" style="cursor:pointer;">
                `;
