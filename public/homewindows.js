            ...
            deswebs.forEach(desweb => {
                if(desweb.titlecss == undefined){
                    ...
                    deswebElement.innerHTML = `
                        <h4>${desweb.titlewindows}</h4>
                        <img src="${desweb.coverwindows}" alt="${desweb.titlewindows}" class="deswebImg" style="cursor:pointer;">
                        <p><strong>Nivel:</strong> ${desweb.nivelwindows}</p>
                    `;
    
                    deswebElement.addEventListener('click', () => {
                        localStorage.setItem('videoDetails', JSON.stringify(desweb));
                        window.location.href = `./itemDetailCss.html?id=${desweb.titlewindows}`;
                    });
        ...
        function displayDeswebs(deswebs) {
            ...
            deswebs.forEach(desweb => {
                ...
                deswebElement.innerHTML = `
                    <h4>${desweb.titlewindows}</h4>
                    <p>Dificultad: ${desweb.nivelwindows}</p>
                    <img src="${desweb.coverwindows}" alt="${desweb.titlewindows}" class="deswebImg" style="cursor:pointer;">
                `;
