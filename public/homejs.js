            ...
            deswebs.forEach(desweb => {
                if(desweb.titlejs == undefined){
                    ...
                    deswebElement.innerHTML = `
                        <h4>${desweb.titlejs}</h4>
                        <img src="${desweb.coverjs}" alt="${desweb.titlejs}" class="deswebImg" style="cursor:pointer;">
                        <p><strong>Nivel:</strong> ${desweb.niveljs}</p>
                    `;
    
                    deswebElement.addEventListener('click', () => {
                        localStorage.setItem('videoDetails', JSON.stringify(desweb));
                        window.location.href = `./itemDetailJs.html?id=${desweb.titlejs}`;
                    });
        ...
        // Funcin para cargar las recetas recin agregadas
        ...
        async function searchDeswebs() {
            // Obtener el valor del input de bsqueda
        ...
        function displayDeswebs(deswebs) {
            ...
            deswebs.forEach(desweb => {
                ...
                deswebElement.innerHTML = `
                    <h4>${desweb.titlejs}</h4>
                    <p>Dificultad: ${desweb.niveljs}</p>
                    <img src="${desweb.coverjs}" alt="${desweb.titlejs}" class="deswebImg" style="cursor:pointer;">
                `;

                deswebElement.addEventListener('click', () => {
                    localStorage.setItem('deswebDetails', JSON.stringify(desweb));
                    window.location.href = `./CSS.html?id=${desweb.id}`;
                });
                deswebsContainer.appendChild(deswebElement);
            });
        }
