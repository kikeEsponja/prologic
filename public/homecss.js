            ...
            deswebs.forEach(desweb => {
                if(desweb.titlecss == undefined){
                    ...
                    deswebElement.innerHTML = `
                        <h4>${desweb.titlecss}</h4>
                        <img src="${desweb.covercss}" alt="${desweb.titlecss}" class="deswebImg" style="cursor:pointer;">
                        <p><strong>Nivel:</strong> ${desweb.nivelcss}</p>
                    `;
    
                    deswebElement.addEventListener('click', () => {
                        localStorage.setItem('videoDetails', JSON.stringify(desweb));
                        window.location.href = `./itemDetailCss.html?id=${desweb.titlecss}`;
                    });
        ...
        function displayDeswebs(deswebs) {
            ...
            deswebs.forEach(desweb => {
                ...
                deswebElement.innerHTML = `
                    <h4>${desweb.titlecss}</h4>
                    <p>Dificultad: ${desweb.nivelcss}</p>
                    <img src="${desweb.covercss}" alt="${desweb.titlecss}" class="deswebImg" style="cursor:pointer;">
                `;
